const assert = require('assert')
let runCallbackAgainstFile = require('../../utils/js/runCbAgainstFileAsArray')
let { solution } = require('./07-1-solution')

describe('07-1-solution', () => {
    describe('solution', () => {
        it('should return 43210 for example input', () => {
            assert.equal(solution('3,15,3,16,1002,16,10,16,1,16,15,15,4,15,99,0,0'), 43210)
        })
        it('should return 54321 for example input', () => {
            assert.equal(solution('3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0'), 54321)
        })
        it('should return 65210 for example input', () => {
            assert.equal(
                solution(
                    '3,31,3,32,1002,32,10,32,1001,31,-2,31,1007,31,0,33,1002,33,7,33,1,33,31,31,1,32,31,31,4,31,99,0,0,0'
                ),
                65210
            )
        })
        it('should return 65210 for example input as file', () => {
            let result = runCallbackAgainstFile(solution, '2019/07-intcode-patch2/example.txt')
            assert.equal(result, 65210)
        })
        it('should return 199988 for puzzle input', () => {
            let result = runCallbackAgainstFile(solution, '2019/07-intcode-patch2/input.txt')
            assert.equal(result, 199988)
        })
    })
})
